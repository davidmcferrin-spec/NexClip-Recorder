/**
 * nexrec-scopes.js — Live confidence waveform + vectorscope.
 *
 * Samples the decoded preview <video> (WHEP) via requestVideoFrameCallback.
 * Rec.709 Y′CbCr, IRE = Y′×100 (post-browser YUV→RGB; treat as 0–100 IRE).
 * 75% Rec.709 bar boxes + skin-tone I-line on the vectorscope.
 * Never burned into the record encode. Not an SDI / Blackmagic QC scope.
 *
 * Click the strip to pop a ~2× panel onto the page (escapes overflow-hidden
 * panes). Esc or click again docks. Popped panel is page-fixed and draggable;
 * a short move does not dock.
 * Per-browser prefs (default off, docked):
 *   nexrec-scopes-on   1 | 0
 *   nexrec-scopes-pop  1 | 0
 *   nexrec-scopes-pos  {"left":N,"top":N}
 */
(function (global) {
  "use strict";

  const PREF_ON = "nexrec-scopes-on";
  const PREF_POP = "nexrec-scopes-pop";
  const PREF_POS = "nexrec-scopes-pos";
  const FADE = 0.22;
  ...
  /**
   * Overlay one sampled RGBA frame onto waveform + vectorscope trace buffers.
   * wfmD / vecD are packed RGBA, length wfmW*wfmH*4 and vecSize*vecSize*4.
   */
  function plotFrame(data, sampleW, L, wfmD, vecD) {
    const vcx = L.vecSize / 2;
    const vcy = L.vecSize / 2;
    const vr = L.vecSize / 2 - L.vecInset;
    const sw = sampleW || L.sampleW;
...
.nexrec-scopes[hidden] { display: none !important; }
.nexrec-scopes.nexrec-scopes-pop {
...
.nexrec-scopes.nexrec-scopes-pop.nexrec-scopes-drag { cursor: grabbing; }
.nexrec-scopes-pane {
...
.nexrec-scopes.nexrec-scopes-pop .nexrec-scopes-pane { padding: 5px 6px 4px; }
.nexrec-scopes-label {
...
`;
  ...
  function attach(opts) {
    ...
    const root = el("div", "nexrec-scopes");
    root.hidden = true;
    root.setAttribute("role", "button");
    root.setAttribute("tabindex", "0");
    const wfmPane = el("div", "nexrec-scopes-pane");
    wfmPane.appendChild(el("div", "nexrec-scopes-label", "WFM · IRE"));
    const vecPane = el("div", "nexrec-scopes-pane");
    vecPane.appendChild(el("div", "nexrec-scopes-label", "VEC · 75%"));
    const wfm = makeCanvas(L.wfmW, L.wfmH);
    const vec = makeCanvas(L.vecSize, L.vecSize);
    wfmPane.appendChild(wfm);
    vecPane.appendChild(vec);
    root.appendChild(wfmPane);
    root.appendChild(vecPane);
    ...
    function applyHost() {
      ...
      root.classList.toggle("nexrec-scopes-pop", !!(popped && visible));
      root.classList.toggle("nexrec-scopes-drag", !!(drag && drag.moved));
    ...
    function sampleFrame() {
      ...
      plotFrame(data, L.sampleW, L, wfmImg.data, vecImg.data);
    ...
    function endDrag(ev) {
      ...
      drag = null;
      root.classList.remove("nexrec-scopes-drag");
