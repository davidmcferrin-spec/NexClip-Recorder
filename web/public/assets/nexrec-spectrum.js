/**
 * nexrec-spectrum.js — Live stereo 64-band RTA (confidence, not a QC RTA).
 *
 ...
 * Popped panel is page-fixed and draggable; a short move does not dock.
 * Per-browser prefs (default off, docked):
 *   nexrec-spectrum-on    1 | 0
 *   nexrec-spectrum-pop   1 | 0
 *   nexrec-spectrum-pos   {"left":N,"top":N}
 */
(function (global) {
  "use strict";

  const PREF_ON = "nexrec-spectrum-on";
  const PREF_POP = "nexrec-spectrum-pop";
  const PREF_POS = "nexrec-spectrum-pos";
  const BANDS = 64;
  const F_MIN = 10;
  const F_MAX = 22000;
  const DB_MIN = -60;
...
.nexrec-spectrum[hidden] { display: none !important; }
.nexrec-spectrum.nexrec-spectrum-pop {
  position: fixed; left: 12px; bottom: 12px; z-index: 55;
  transform: none; width: auto;
  cursor: grab;
  touch-action: none;
}
.nexrec-spectrum.nexrec-spectrum-pop.nexrec-spectrum-drag { cursor: grabbing; }
.nexrec-spectrum-pane {
  background: rgba(8, 12, 16, .78);
  border: 1px solid rgba(44, 53, 66, .9);
  border-radius: 3px;
  padding: 4px 5px 3px;
}
.nexrec-spectrum.nexrec-spectrum-pop .nexrec-spectrum-pane { padding: 5px 6px 4px; }
.nexrec-spectrum-label {
...
`;
  ...
  function attach(opts) {
    ...
    root.className = "nexrec-spectrum";
    root.hidden = true;
    root.setAttribute("role", "button");
    root.setAttribute("tabindex", "0");
    const pane = document.createElement("div");
    pane.className = "nexrec-spectrum-pane";
    const label = document.createElement("div");
    label.className = "nexrec-spectrum-label";
    label.textContent = "RTA · L/R · 64-band · −60…0 dBFS";
    label.title = LIMIT;
    ...
    function applyHost() {
      ...
      root.classList.toggle("nexrec-spectrum-pop", !!(popped && visible));
      root.classList.toggle("nexrec-spectrum-drag", !!(drag && drag.moved));
      root.title = (popped ? "Drag to move · click to dock · Esc. " : "Click to enlarge. ") + LIMIT;
      root.setAttribute("aria-pressed", popped ? "true" : "false");
      root.setAttribute("aria-label", popped ? "Dock spectrum analyzer" : "Enlarge spectrum analyzer");
      applyPosition();
    }
    ...
    function paintLabel() {
      let text = "RTA · L/R · 64-band · −60…0 dBFS";
      if (vu && typeof vu.getMonitorNote === "function") {
        const note = vu.getMonitorNote();
        if (note && note.indexOf("stereo") >= 0) text = "RTA · L/R · 64-band · stereo proxy";
        else if (note && note.indexOf("waiting") >= 0) text = "RTA · L/R · 64-band · −60…0 dBFS";
        else if (note) text = "RTA · listen pair · 64-band · " + note;
      }
      if (text !== labelText) {
        labelText = text;
    ...
    function endDrag(ev) {
      ...
      drag = null;
      root.classList.remove("nexrec-spectrum-drag");
